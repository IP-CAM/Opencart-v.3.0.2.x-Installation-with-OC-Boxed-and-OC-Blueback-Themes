...
class ModelCatalogSphinx extends Model
{
    ...
    public function getSphinxClient()
    {
        if (null === $this->_sphinxClient) {
            ...
            $sphinxClient = new SphinxClient();
  		    $sphinxClient->SetServer($this->config->get('module_sphinxsearch_server'),
  		                             $this->config->get('module_sphinxsearch_port'));
    ...
    public function search($data, $index = null, $wildcard = false)
    {
        ...
        $result = array ('product_total' => 0,
                         'results' => array(),
        				 'lastQueryKeywords' => $this->getLastQueryKeywords());
        ...
        if ($fieldWeights = $this->config->get('module_sphinxsearch_weights')) {
        ...
        // Set matching mode
        if (!$wildcard) {
            $sphinxClient->SetMatchMode((int)$this->config->get('module_sphinxsearch_match_mode'));

            // Set how to rank the weighted values
            $sphinxClient->SetRankingMode((int)$this->config->get('module_sphinxsearch_ranking_mode'));
        } else {
        ...
        // Set quantity filter
        if ($index == 'products') {
            $quantity = (int)$this->config->get('module_sphinxsearch_products_quantity');
            $sphinxClient->SetFilterRange('quantity', $quantity, 2147000000);
        }

        // Set product status filter
        if((int)$this->config->get('module_sphinxsearch_product_status') == 0) {
        ...
        // Set page limits
        $sphinxClient->SetLimits(abs((int)$data['start']), abs((int)$data['limit']));
        ...
        if (isset($searchResults['words'])) {
            ...
            $result['lastQueryKeywords'] = $this->getLastQueryKeywords();
        }
