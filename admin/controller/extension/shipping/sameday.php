    ...
    public function addAwb()
    {
        ...
        if ($this->request->server['REQUEST_METHOD'] === 'POST' && $this->validateFormBeforeAwbGeneration()) {
            $params = array_merge($this->request->post, $orderInfo);
						if (isset($params['shipping_custom_field']['11']) && !empty($params['shipping_custom_field']['11'])) {
							$params['telephone'] = $params['shipping_custom_field']['11'];
						}
						if (isset($params['shipping_company']) && !empty($params['shipping_company'])) {
							$params['payment_company'] = $params['shipping_company'];
						}
        ...
        if ($orderInfo['payment_code'] == 'euplatesc') {
			$data['sameday_ramburs'] = 0;
		} else {
			$data['sameday_ramburs'] = round($orderInfo['total'], 2);
		}

		if (empty($data['sameday_observation'])) {
			$data['sameday_observation'] = $this->language->get('default_observation');
		}

		if ($data['sameday_package_type'] == '') {
			$data['sameday_package_type'] = 1;
		}

        if ($data['sameday_insured_value'] == '') {
            $data['sameday_insured_value'] = 0;
        }
