	...
	public function index($setting) {
		...
		if(is_array($setting['tabs'])) {
			...
			foreach($tabs as $tab) {
				...
				// Najnowsze produkty, Specjalne produkty, Najlepiej sprzedajace siÄ produkty, Wybrane produkty z kategorii
				if($tab['title'] == 'latest' || $tab['title'] == 'special' || $tab['title'] == 'bestsellers' || $tab['title'] == 'category' || $tab['title'] == 'random' || $tab['title'] == 'people_also_bought' || $tab['title'] == 'most_viewed' || $tab['title'] == 'related') {
					...
					} elseif($tab['title'] == 'random') {
					          $this->load->model('catalog/products');
							$results = $this->model_catalog_products->getRandomProducts($setting['limit']);
					} elseif($tab['title'] == 'people_also_bought') {
					          $this->load->model('catalog/products');
							$results = $this->model_catalog_products->getAlsoBoughtProducts($setting['limit']);
					} elseif($tab['title'] == 'most_viewed') {
					          $this->load->model('catalog/products');
							$results = $this->model_catalog_products->getMostViewedProducts($setting['limit']);
					} elseif($tab['title']== 'related') {
					     $this->load->model('catalog/products');
					     $results = $this->model_catalog_products->getProductsRelated($setting['limit']);
					}
		...
		return $this->load->view('extension/module/filter_product', $data);
	}
}
?>