	...
	public function getProducts($data = array()) {
                ...
                if(isset($data['filter_special']) || !empty($data['filter_min_special_perc']) || !empty($data['filter_rating']) || !empty($data['filter_min_rating']) || !empty($data['filter_max_rating']) || !empty($data['filter_min_price']) || !empty($data['filter_max_price'])){
                        ...
                        if(in_array('p.price', $sort)){
                            ...
                            $sql .= ", (SELECT price FROM " . DB_PREFIX . "product_special WHERE product_id = p.product_id AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((date_start = '0000-00-00 00:00:00' OR date_start < NOW()) AND (date_end = '0000-00-00 00:00:00' OR date_end > NOW())) ORDER BY priority ASC, price ASC LIMIT 1) AS special";
                        }
			...
			'p.date_added' => 'date_added',
                ...
                if(isset($data['sort_order'])){
                    ...
                    foreach ($data['sort_order'] as $sort_order) {
                        if (in_array($sort_order['sort'], $sort_data) && in_array($sort_order['order'], array('ASC', 'DESC'))) {
                ...
                if($product_table == DB_PREFIX . 'product'){
                    ...
                    if(in_array('price', $additional_field) || !empty($data['filter_min_price']) || !empty($data['filter_max_price']) || isset($data['filter_special'])){
                        if($this->config->get('config_tax')){
                            ...
                            $sql .= ", (SELECT price + IFNULL(ftax.total,0) + ((IFNULL(ptax.total,0) * price) / 100) FROM " . DB_PREFIX . "product_special WHERE product_id = p.product_id AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((date_start = '0000-00-00 00:00:00' OR date_start < NOW()) AND (date_end = '0000-00-00 00:00:00' OR date_end > NOW())) ORDER BY priority ASC, price ASC LIMIT 1) AS special";
                        } else {
                            $sql .= ", p.price";
                            $sql .= ", (SELECT price FROM " . DB_PREFIX . "product_discount WHERE product_id = p.product_id AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND quantity = '1' AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) ORDER BY priority ASC, price ASC LIMIT 1) AS discount";
                            $sql .= ", (SELECT price FROM " . DB_PREFIX . "product_special WHERE product_id = p.product_id AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((date_start = '0000-00-00 00:00:00' OR date_start < NOW()) AND (date_end = '0000-00-00 00:00:00' OR date_end > NOW())) ORDER BY priority ASC, price ASC LIMIT 1) AS special";
                    ...
                    if(in_array('special_perc', $additional_field) || !empty($data['filter_min_special_perc'])){
                        $sql .= ", (SELECT 100 - ((price * 100) / p.price) FROM " . DB_PREFIX . "product_special WHERE product_id = p.product_id AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((date_start = '0000-00-00 00:00:00' OR date_start < NOW()) AND (date_end = '0000-00-00 00:00:00' OR date_end > NOW())) ORDER BY priority ASC, price ASC LIMIT 1) AS special_perc";
                    }
                }
