<?php
class ModelSaleJReturnEmail extends Model {
	public function sendEmailReturnHistory($return_data) {
		/*Order Return email starts*/
		if ($this->config->get('jreturnemail_status') && $this->config->get('jreturnemail_emailtocustomer_history') && $return_data['data']['notify']) {

			$this->load->model('setting/setting');
			$this->load->model('sale/order');
			$this->load->model('tool/image');

			$return_query = $this->db->query("SELECT r.* FROM `" . DB_PREFIX . "return` r WHERE r.return_id = '" . (int)$return_data['return_id'] . "'");
			...
			$catalog_url = new Url(($getconfig('config_url') ? $getconfig('config_url') : HTTP_SERVER), ($getconfig('config_ssl') ? $getconfig('config_ssl') : HTTPS_SERVER));
			...
			if(isset($email_template['customerhistory'][(int)$config_language_id])) {
				$template_customer_mail = $email_template['customerhistory'][(int)$config_language_id];
			} else {
				reset($email_template['customerhistory']);
				$first_key = key($email_template['customerhistory']);
				if (isset($email_template['customerhistory'][$first_key])) {
					$template_customer_mail = $email_template['customerhistory'][$first_key];
				}
			}
			$comment = '';
			if ($return_data['data']['comment']) {
				$comment = strip_tags(html_entity_decode($return_data['data']['comment'], ENT_QUOTES, 'UTF-8'));
			}
			...
			if ($template_customer_mail) {
				...
				$replace = array(
					...
					'PRODUCT_URL' => $product_info['product_id'] ? $catalog_url->link('product/product', 'product_id=' . $product_info['product_id'], true) : '',
				...
				$mail->send();
				// echo "Admin Return Add History Customer E-mail \n\n";
				// print_r($mail);
			}

			}
			// update variable, so default mail not goes
			$return_data['data']['notify'] = 0;

		}
	}

	public function sendEmailReturnAction($return_data) {
		/*Order Return email starts*/
		if ($this->config->get('jreturnemail_status') && $this->config->get('jreturnemail_emailtocustomer_action') && (!empty($this->config->get('jreturnemail_return_action')) && is_array($this->config->get('jreturnemail_return_action'))) && in_array($return_data['data']['return_action_id'], $this->config->get('jreturnemail_return_action')) ) {
			...
			$template_customer_mail = array();
			if(isset($email_template['customeraction'][(int)$config_language_id])) {
				$template_customer_mail = $email_template['customeraction'][(int)$config_language_id];
			} else {
				reset($email_template['customeraction']);
				$first_key = key($email_template['customeraction']);
				if (isset($email_template['customeraction'][$first_key])) {
					$template_customer_mail = $email_template['customeraction'][$first_key];
				}
			}
			...
			if ($template_customer_mail) {
				...
				$mail->setHtml(html_entity_decode($message, ENT_QUOTES, 'UTF-8'));
				$mail->send();
				// echo "Admin Return Action Customer E-mail \n\n";
				// print_r($mail);
	...
	public function sendEmailReturnAdd($return_data) {
		/*Order Return email starts*/
		if ($this->config->get('jreturnemail_status')) {
			...
			$replace = array(
				...
				'PRODUCT_NAME' => $return_info['product'],
				'PRODUCT_URL' => $product_info['product_id'] ? $this->url->link('catalog/product/edit', 'product_id=' . $product_info['product_id'], true) : '',
				'PRODUCT_MODEL' => $return_info['model'],
				'PRODUCT_QTY' => $return_info['quantity'],
				'PRODUCT_OPENED_STATUS' => $return_info['opened'] ? $this->language->get('text_yes') : $this->language->get('text_no'),
				'PRODUCT_THUMB' => $product_thumb ? '<img src="'.  $product_thumb .'" alt="'.$return_info['product'].'" />' : '',
			...
			$mail->setHtml(html_entity_decode($message, ENT_QUOTES, 'UTF-8'));
			$mail->send();
			// echo "Admin Return Add Admin E-mail \n\n";
			// print_r($mail);
			}
			if ($this->config->get('jreturnemail_emailtocustomer')) {
				...
				$mail->setHtml(html_entity_decode($message, ENT_QUOTES, 'UTF-8'));
				$mail->send();
				// echo "Admin Return Add Customer E-mail \n\n";
				// print_r($mail);
