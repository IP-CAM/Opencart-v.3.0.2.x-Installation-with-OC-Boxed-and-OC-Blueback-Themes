...
class ModelCatalogSphinxsearch extends ModelCatalogSphinx {
	...
	public function install() {
		...
		$this->db->query("
        CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "module_sphinxsearch_suggestions` (
	...
	public function sphinxConnection() {
		...
		if (null === $this->_sphinxConnection) {
			$sphinxConnection = new mysqli($this->config->get('module_sphinxsearch_server'), DB_USERNAME, DB_PASSWORD, DB_DATABASE, 9306, '');
		...
		foreach ($data as $lang_id => $arr) {
			...
			foreach ($keywords as $key => $arr) {
				...
				if(!$this->config->get('module_sphinxsearch_config_index_type')) {
					$sql = "INSERT IGNORE INTO `" . DB_PREFIX . "module_sphinxsearch_suggestions` (`keyword`, `trigrams`, `freq`)
